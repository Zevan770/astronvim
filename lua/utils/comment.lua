---@author chrisgrieser
---@source https://github.com/chrisgrieser/.config/blob/main/nvim/lua/personal-plugins/comment.lua?plain=1
local M = {}
--------------------------------------------------------------------------------
...
function M.setupReplaceModeHelpersForComments()
  vim.api.nvim_create_autocmd("ModeChanged", {
    ...
    callback = function(ctx)
      ...
      if vim.startswith(vim.trim(line), comChars) then vim.api.nvim_set_current_line(line:upper()) end
    end,
  })
  vim.api.nvim_create_autocmd("ModeChanged", {
    ...
    callback = function(ctx)
      ...
      local line = vim.api.nvim_get_current_line()
      local comChars = vim.trim(vim.bo.commentstring:format "")
      if vim.startswith(vim.trim(line), comChars) then vim.cmd.normal { "^" .. #comChars + 1 .. "l", bang = true } end
    end,
  })
end
...
function M.commentHr(replaceModeLabel)
  ...
  if not replaceModeLabel then vim.api.nvim_buf_set_lines(0, startLn + 1, startLn + 1, true, { "" }) end
