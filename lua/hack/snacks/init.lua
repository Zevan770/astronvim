...
function H.truncate(text, width, direction)
  if width <= 1 then return width == 1 and "…" or "" end
...
return {
            ...
            actions = {
              filter_extension = function(picker)
                ...
                Snacks.input.input({
                  ...
                  if mode == "n" then vim.cmd.stopinsert() end
                end)
              end,
            },
            win = {
          ...
          list = {
            keys = {
              ["<Esc>"] = {
                "<Esc>",
                function(self)
                  if not U.keymap.clear_ui_esc { close = false } then self:execute "toggle_focus" end
                end,
  ...
  },
  {
    ...
    opts = function()
      ...
      Snacks.picker.util.truncpath = function(path) return path end
      ...
      Snacks.picker.format.filename = function(item, picker)
        ...
        if picker.opts.formatters.file.filename_only then return ret end
        ...
        if prefix then
          for _, text in ipairs(ret) do
            if text[2] ~= "SnacksPickerDir" then prefix = prefix + vim.api.nvim_strwidth(text[1]) end
            if text[2] == "SnacksPickerFile" or text[2] == "SnacksPickerDirectory" then break end
          end
...

  {
    ...
    opts = function(_, opts)
      if LazyVim.pick.picker.name ~= "snacks" then return end
      ...
      defaults = {
        ...
        layout_config = {
          ...
          vertical = {
            ...
            height = function(_, _, max_lines) return vim.g.user_is_termux and max_lines or math.floor(max_lines * 0.8) end,
            preview_cutoff = 20,
            preview_height = 0.5,
          },
        },
    ...
    dependencies = {
      {
        ...
        init = function()
          LazyVim.on_load("telescope.nvim", function() require("telescope").load_extension "lazy_plugins" end)
        end,
      },
    },
    keys = {
