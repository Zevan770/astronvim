{
  # modified from https://github.com/ayamir/nvimdots/blob/main/flake.nix
  ...
  description = "Provide nixosModules for ZeVan's neovim configuration";
        ...
        devshells.default = {
          ...
          devshell = {
            motd = ''
              ...
              Symlink configs to "''${XDG_CONFIG_HOME}"/zvim!
              And NVIM_APPNAME=zvim is already configured, so neovim will put files under "\$XDG_xxx_HOME"/zvim.
              To uninstall, remove "\$XDG_xxx_HOME"/zvim.
            ...
            startup = {
              mkNvimDir = {
                text = ''
                  mkdir -p "''${XDG_CONFIG_HOME}"/zvim
                  for path in lazy-lock.json init.lua lua snips tutor; do
                    ln -sf "''${PWD}/''${path}" "''${XDG_CONFIG_HOME}"/zvim/
                  done
                '';
              };
            };
